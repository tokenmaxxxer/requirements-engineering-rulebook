#!/usr/bin/env bash
. "${CLAUDE_PLUGIN_ROOT_CORE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../core" && pwd -P)}/hooks/lib/gate-lib.sh" || { echo "ambiguity-resolution-gate.sh: cannot source gate-lib.sh" >&2; exit 2; }
...
# docs/issue-1/proposals/requirements-engineering.md (b)(3)/(c): the
# deliverable record (docs/issue-<n>/reports/requirements-engineering.md)
# must carry an "ambiguity" section, and that section must either state
...
try:
    ...
    if not isinstance(ti, dict):
        deny("tool_input is missing or not a JSON object; the gate cannot judge a write it cannot parse (ambiguity facet).")
    ...
    if not ok:
        deny(
            ...
            "Edit/MultiEdit whose old_string matches, so the ambiguity facet can be "
            "checked." % (rel, tool)
        )
    ...
    if "ambiguity" not in low:
        deny("ambiguity facet: no 'ambiguity' section found")
    elif not (
        has_any("no ambiguities found", "none found", "no ambiguities")
        or has_any("resolution:", "escalated")
    ):
        deny("ambiguity facet: no explicit 'none found' statement and no resolved/escalated entry found")
    else:
        sys.exit(0)

    sys.exit(0)
except Exception as _fc_e:  # fail-closed-on-internal-error
    _fc_sys.stderr.write("ambiguity-resolution-gate.sh: fail-closed: internal error: %r\n" % (_fc_e,))
    _fc_sys.exit(2)
PY
