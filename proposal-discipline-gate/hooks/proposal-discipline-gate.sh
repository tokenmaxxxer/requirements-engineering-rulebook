...
#
# Kill switch: export PROPOSAL_DISCIPLINE_GATE_OFF=1
set -uo pipefail

role="${CLAUDE_ROLE:-requirements-engineering}"
deny() { echo "requirements-engineering: refused — $1" >&2; exit 2; }

case "${PROPOSAL_DISCIPLINE_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac

command -v python3 >/dev/null 2>&1 || deny "proposal-discipline-gate.sh requires python3, which is not on PATH; denying rather than guessing."

payload="$(cat 2>/dev/null || true)"
[ -n "$payload" ] || deny "proposal-discipline-gate: empty tool-use payload on stdin; cannot evaluate the proposal-structure gate."
...
[ -z "$root" ] && deny "no project root could be determined; failing closed (proposal-structure check cannot run)."
...
try:
    ...
    try:
        ...
        deny("the tool-call payload is not valid JSON; the gate cannot judge proposal structure on an unparseable write.")
    if not isinstance(ev, dict):
        deny("the tool-call payload is not a JSON object; failing closed on proposal structure.")
